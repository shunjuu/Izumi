...
"$ffmpeg8" -i "$tempfile" -vf subtitles="$tempfile" -c:a aac -threads $THREADS -y -nostdin -strict -2 "$hardsubfile"
...
if [[ "$newsize" == "0" ]]; then
		...
		"$ffmpeg10" -i "$tempfile" -vf subtitles="$tempfile" -c:a aac -threads $THREADS -y -nostdin -strict -2 "$hardsubfile"
fi
